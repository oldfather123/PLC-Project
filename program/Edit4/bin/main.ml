...
let () =
  if Array.length Sys.argv < 2 then (
    ...
    Printf.eprintf "  test ir            - run IR generation tests\n";
    Printf.eprintf "  test features      - run specific IR feature tests\n";
    ...
    Printf.eprintf "  ir <file>          - generate and show three-address code\n";
    exit 1
  );
  ...
  | "ir" when Array.length Sys.argv >= 3 ->
    let filename = Sys.argv.(2) in
    (try
      let ast = parse_file filename in
      Printf.printf "=== Three-Address Code for: %s ===\n\n" filename;
